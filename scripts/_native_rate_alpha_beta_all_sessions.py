"""Batch alpha+beta S-map on all sessions with available XDFs.

Same convention as theta runner: per-session JSONs go into
results/native_rate_coupling_p2/alpha_beta_grand_avg/.
"""
import os
import sys
import subprocess
...
OUT_DIR = os.path.join(ROOT, 'results', 'native_rate_coupling_p2', 'alpha_beta_grand_avg')
...
def run_session(session_arg):
    ...
    if os.path.exists(out_json):
        print(f'[SKIP] {session_arg}: output exists')
        return True
    print(f'[RUN ] {session_arg}: alpha+beta S-map ...', flush=True)
    t0 = time.time()
    env = os.environ.copy()
