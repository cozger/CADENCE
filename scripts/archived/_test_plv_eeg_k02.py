"""PLV on real EEG, kappa=0.2, spatial decay, avg-ref."""
import sys, os, time
import numpy as np
...
FS = 256.0; DURATION = 1800; N_CH = 14; KAPPA = 0.2; SEED = 42
...
print(f"kappa=0.2 spatial decay: {len(oracle_ch)} oracle channels")
print(f"  {[(EPOC_CHANNEL_NAMES[i], f'{kappa_per_ch[i]:.3f}') for i in oracle_ch]}")

freqs = np.logspace(np.log10(2.0), np.log10(40.0), 30)
t0 = time.perf_counter()
mask, z_agg, pcz, diag = wpli_temporal_localization(
    p1_c, p2_coupled, FS, channels=oracle_ch,
...
print(f"\n  z_coupled={zc:.3f}, z_null={zn:.3f} ({elapsed:.0f}s)")
print(f"\n  {'Thr':>6} {'Hit':>7} {'FA':>7}")
